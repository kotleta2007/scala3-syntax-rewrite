...
case class Scala3ControlSyntaxParameters(
  useCatchInlining: Boolean
)

object Scala3ControlSyntaxParameters {
  val default = Scala3ControlSyntaxParameters(false)
  implicit val surface: metaconfig.generic.Surface[Scala3ControlSyntaxParameters] = metaconfig.generic.deriveSurface[Scala3ControlSyntaxParameters]
  implicit val decoder: metaconfig.ConfDecoder[Scala3ControlSyntaxParameters] = metaconfig.generic.deriveDecoder(default)
}

class Scala3ControlSyntax(params: Scala3ControlSyntaxParameters)
    extends SyntacticRule("Scala3ControlSyntax") {
  
  def this() = this(Scala3ControlSyntaxParameters.default)

  override def withConfiguration(config: Configuration): Configured[Rule] = 
    config.conf.getOrElse("Scala3ControlSyntax")(this.params).map(newParams => new Scala3ControlSyntax(newParams))
